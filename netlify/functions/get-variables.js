// Netlify Function: /.netlify/functions/get-variables
// GET /projects/{siteUuid}/variables
...
export async function handler(event, context) {
  ...
  try {
    ...
    const siteUuid = pathParts[pathParts.indexOf('projects') + 1];
    
    if (!siteUuid) {
      return {
        statusCode: 400,
        headers,
        body: JSON.stringify({ success: false, error: 'Site UUID required' })
      };
    }
    ...
    if (projectData) {
      await supabase
        ...
        .insert({
          ...
          endpoint: '/projects/' + siteUuid + '/variables',
    ...
    return {
      statusCode: 200,
      headers,
      body: JSON.stringify({ success: true, data: variables })
    };
